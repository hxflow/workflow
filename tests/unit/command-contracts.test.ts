...
describe('command contracts', () => {
  ...
  it('keeps the main-chain prompts minimal and aligned', () => {
    ...
    expect(hxReview).toContain('必须先有已配置 gate')
    expect(hxReview).not.toContain('--scope')
    ...
    expect(hxInit).toContain('不做项目分析')
    expect((hxInit.match(/\n1\./g) ?? []).length).toBe(1)
    ...
    expect(hxMr).toContain('git status --porcelain')
    expect(hxMr).toContain('未提交修改必须先提交')
    expect(hxMr).toContain('不允许跳过提交直接创建 MR')

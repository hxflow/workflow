...
# 3. 开始开发
# Claude: /hx-go --task 12345 --profile base
# Codex:  hx-go --task 12345 --profile base
```
...
| 命令 | 用途 |
|------|------|
| `hx-go [feature-key] [--task <id>] [--from <step>] --profile <name>` | 全自动流水线，默认串起 `doc -> plan -> run -> qa -> mr` |
...
- `hx-ctx`、`hx-review`、`hx-fix`、`hx-clean` 都属于按需使用的辅助能力，不是每次交付都必须手动执行
